      ...
      <form action="{$same_page}" method="post">
        ...
        <tr>
          ...
          <td rowspan="2" valign="top">
            <input type="checkbox" name="vis_types[]" id="vt1" value="activity" {if "activity"|in_array:$vis_types}checked{/if} />
              <label for="vt1">{$L.phrase_activity_charts}</label><br />
            <input type="checkbox" name="vis_types[]" id="vt2" value="field" {if "field"|in_array:$vis_types}checked{/if} />
              <label for="vt2">{$L.phrase_field_charts}</label>
          </td>
        </tr>
        <tr>
          <td class="medium_grey">{$LANG.word_form}</td>
          <td class="col2">{forms_dropdown name_id="form_id" default=$search_form_id include_blank_option=true blank_option_label=$L.phrase_all_forms}</td>
        </tr>
        <tr>
          <td class="medium_grey">{$LANG.word_view}</td>
          <td class="col2">
            {if $search_form_id}
              {views_dropdown form_id=$search_form_id name_id="view_id" selected=$search_view_id
          ...
          <td>
            <select name="chart_type">
              ...
              <option value="area_chart"   {if $chart_type == "area_chart"}selected{/if}>{$L.word_area}</option>
              <option value="bar_chart"    {if $chart_type == "bar_chart"}selected{/if}>{$L.word_bar}</option>
              <option value="column_chart" {if $chart_type == "column_chart"}selected{/if}>{$L.word_column}</option>
              <option value="line_chart"   {if $chart_type == "line_chart"}selected{/if}>{$L.word_line}</option>
              <option value="pie_chart"    {if $chart_type == "pie_chart"}selected{/if}>{$L.word_pie}</option>
            </select>
          </td>
        </tr>
        <tr>
            ...
            <tr>
              <td>
                <input type="radio" name="account_type" value="admin" id="at1"
                  {if $account_type == "admin"}checked{/if} /> <label for="at1">{$LANG.word_administrator}</label>
              </td>
            </tr>
            <tr>
              <td>
