  <form action="{$same_page}" method="post" onsubmit="return rsv.validate(this, rules)">
    ...
    <tr>
      ...
      <td>
        <input type="text" name="vis_name" id="vis_name" value="{$vis_info.vis_name|escape}" style="width: 99%" />
      </td>
    </tr>
    <tr>
    ...
    <tr>
      ...
      <td>
        ...
        {foreach from=$form_fields item=field_info}
          <option value="{$field_info.field_id}" {if $field_info.field_id == $vis_info.field_id}selected{/if}>{$field_info.field_title}</option>
        {/foreach}
        </select>
      </td>
    </tr>
